...
| # | 建議修復 | 驗證方式 |
| - | - | - |
| S6-1 | MIMO mode 下清空或摺疊 Phase 7 SISO-only 舊結果；若使用者點 SISO-only 按鈕，輸出區改顯示「此功能僅適用 SISO，請使用 MIMO Analysis → MIMO LQR」 | Browser walkthrough：切 SISO 算 LQR → 切 MIMO → Phase 7 output 不再顯示舊 K |
| S6-2 | MIMO mode 下 Advisor 預設將 MIMO Analysis 排到 Phase 7/8 前，或新增 quick jump | Browser walkthrough：切 MIMO 後一進 Advisor 即可看到 MIMO Analysis |
| S6-3 | RGA / W / G(0)W 改成矩陣 table，row label=`y_i`、column label=`u_j` | Browser smoke：RGA table cell 顯示 `λ11=1.333` 且 row/column label 存在 |
| S6-4 | Singular Value Bode 提供全寬或較高 chart，legend 明確包含 `σ_max` / `σ_min` | Browser smoke：chart height >= 260px，legend text 可見 |
| S6-5 | PID slider 旁新增 numeric input，slider 與 input 雙向同步 | Browser smoke：輸入 `Kp=2.5` 後 controller equation 同步更新 |
| S6-6 | LQR output 補 solver path 說明：Hamiltonian Schur / Newton-Kleinman fallback / zero-gain stable-A | Browser smoke：MIMO LQR output 包含 `Solver:` 與 residual |
...
但在進入大量新 UI 前，建議先處理 S6-1，因為它會讓 MIMO 使用者誤讀 SISO 舊結果，屬於高風險介面問題。
