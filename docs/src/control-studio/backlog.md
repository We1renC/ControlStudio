...
- Branch: `main`
- Latest synced checkpoint: `fix(zero-flaw): preserve structural minreal rank`
- Current checkpoint: **CS-P0 ~ CS-P77 done; Zero-Flaw Loop 1~16 done; Functional Roadmap Tier A-J done; Phase 19/20/21/23 project-local skill gaps closed; nonlinear equilibrium classification, nonlinear grid-scan hardening, continuous analysis-grid hardening, continuous-analysis domain guards, continuous frequency/robust domain guards, P41 discretization comparison API-contract repair, runtime UI symbol contract enforcement, UI waveform response/metrics contract, UI stability snapshot contract, UI simulation snapshot / HIL export contract, UI analysis snapshot freshness contract, UI discrete domain-switch snapshot/header contract, UI effective loop-mode contract, UI DTF `z^-1` formula display contract, UI DTF sample-time codegen/export/project persistence contract, MATLAB/Python codegen runtime-mode/domain compatibility contract, discrete analysis export response-type and non-finite margin status contract, deployment readiness gate, deployment reviewer skill, deterministic ADP/LSTD-Q/DRO/UIO verification, ESPRIT complex rotation-phase recovery, matrix numerical-health correctness, system-wizard workspace integration, exact continuous/discrete Gramian diagnostics, coupled Hankel singular values, square-root balancing orientation, honest BT/HNA error contracts, Hurwitz Hankel gating, exact zero-HSV/rank semantics, direct Kalman-matrix minreal rank, and sub-resolution Hankel-error semantics closed; verification aggregation closed.** 詳細執行看板見 `control-studio/ROADMAP.md`。目前僅暫停項目維持不做：教學模式、Electron packaging、報告模板 / 報告自動化、Block Diagram expansion。
...
- Full suite baseline：`npm run verify:all` / `bash control-studio/scripts/run_all_verify.sh` 目前納入 131 個 deterministic verification scripts，包含 fixture、API contract、runtime UI waveform contract、runtime UI stability snapshot contract、runtime UI simulation snapshot / discrete-domain / effective-loop contract、runtime UI formula display contract、codegen export contract、discrete export response contract、deployment readiness gate、deployment reviewer skill gate、runtime UI symbol contract、n-dimensional equilibrium classification regression，以及 Zero-Flaw Loop 1~16 literature/algorithm/UI integration coverage；fixture/API contract 目前為 10/10 cases。
...
| CS-P0-28 | P0 | Done | Zero-final-change step metrics rejection | `stepInfo()` 對 net final response change 幾乎為零、但 transient excursion 明顯存在的 step response 不再輸出 normalized rise/settling/overshoot；保留 SSE 並回傳 `valid:false`，避免 zero-DC-gain plant 產生荒謬 overshoot | `stability.js`, `verification-cases.js`, `verify_control_cases.mjs`, `verify_control_api_contract.mjs`, `control_regression_dashboard.mjs` | `node control-studio/scripts/verify_control_cases.mjs`; `node control-studio/scripts/verify_control_api_contract.mjs` |
| CS-P0-29 | P0 | Done | Divergent / unfinished step metrics rejection | `stepInfo()` 若沒有 explicit final value，必須先確認 response tail 已收斂；`G(s)=1/(s-1)` 這類 unit-step 發散響應不再用最後一個 sample 偽造 rise/settling/overshoot，而是回傳 `valid:false` 與 settled-tail reason | `stability.js`, `verification-cases.js`, `verify_control_cases.mjs`, `verify_control_api_contract.mjs`, `control_regression_dashboard.mjs` | `node control-studio/scripts/verify_control_cases.mjs`; `node control-studio/scripts/verify_control_api_contract.mjs` |
| CS-P0-30 | P0 | Done | UI waveform routing and local metrics contract | Local JS UI 對 sine / square / pulse 改走 `simulateTimeResponse()`，不再 fallback 成 step；UI / export / sweep / report 的 step metrics 只對 step input 有效，且使用 configured amplitude 作為 reference；report 產生器也用同一個 amplitude 模擬 step response，避免非單位 step 報告用 unit-step trajectory 對 non-unit reference 算 metrics | `app.js`, `js/ui/sweep.js`, `js/ui/share.js`, `verify_ui_waveform_contract.mjs` | `node control-studio/scripts/verify_ui_waveform_contract.mjs`; `npm run verify:all` |
| CS-P0-31 | P0 | Done | UI stability snapshot and gain-margin field contract | `updateStabilityPanel()` 會發布 canonical `_lastStability`；GM canonical 欄位統一為 `gainMarginDB`，`gainMarginDb` 僅保留相容 alias；flow / warnings / summary / report 透過 helper 正規化 GM，report 將 infinite GM 顯示為 `∞` 並視為通過 `> 6 dB` | `app.js`, `js/ui/flow.js`, `js/ui/share.js`, `verify_ui_stability_snapshot_contract.mjs` | `node control-studio/scripts/verify_ui_stability_snapshot_contract.mjs`; browser report walkthrough; `npm run verify:all` |
| CS-P0-32 | P0 | Done | UI simulation snapshot and HIL export contract | active time-domain plot 會發布 `_lastSimResult` 並 emit `simulation:done`；Result Summary / Warnings Panel / HIL CSV export 讀同一份已繪製 response；companion charts 不覆寫主圖快照；snapshot 保留 `t/y/u`、command input、disturbance input、waveform、domain、loop mode 與 metrics，discrete step chart 也發布同一契約 | `app.js`, `verify_ui_simulation_snapshot_contract.mjs`, `run_all_verify.sh` | `node control-studio/scripts/verify_ui_simulation_snapshot_contract.mjs`; browser HIL export walkthrough; `npm run verify:all` |
| CS-P0-33 | P0 | Done | UI analysis snapshot freshness contract | `updateStabilityPanel()` / time-metric refresh 也會以 explicit `allowNonChartSnapshot` 發布目前 response snapshot；使用者停在 Bode / Nyquist / Nichols / Root Locus / Pole-Zero 等非時域圖時調整 plant、controller 或 simulation config，不會讓 HIL CSV、Result Summary 或 Warnings Panel 沿用舊 `_lastSimResult`；companion charts 仍不能覆寫 canonical simulation state | `app.js`, `verify_ui_simulation_snapshot_contract.mjs` | `node control-studio/scripts/verify_ui_simulation_snapshot_contract.mjs`; browser Bode-to-HIL freshness walkthrough; `npm run verify:all` |
| CS-P0-34 | P0 | Done | UI discrete domain-switch snapshot/header contract | DTF/z-domain plant 更新會清掉不相容的 continuous controller / open-loop / closed-loop / 2-DOF / stability / simulation state，並在 refresh 後跑 `updateStabilityPanel()` 發布目前離散 step snapshot；Bode / Pole-Zero active 時 HIL CSV 不會沿用舊 s-domain response；companion discrete step chart 不會覆寫 active plot header | `app.js`, `verify_ui_simulation_snapshot_contract.mjs` | `node control-studio/scripts/verify_ui_simulation_snapshot_contract.mjs`; browser continuous-Bode → DTF-Bode HIL freshness walkthrough; `npm run verify:all` |
| CS-P0-35 | P0 | Done | UI effective loop-mode and DTF formula display contract | Runtime paths 只在實際存在 `state.closedLoop` 時回報 `closed_loop`，status bar、snapshot、codegen、API payload、comparison/export、AI Advisor 與 smoke state 不再把 closed-loop toggle preference 誤當成有效模型；DTF 公式顯示採用 `G(z)` / `L(z)` / `T(z)` 與 `z^-1` delay-polynomial convention，active discrete step 顯示 legend，open-loop smoke 不強制要求 closed-loop formula | `app.js`, `verify_ui_simulation_snapshot_contract.mjs`, `verify_ui_formula_contract.mjs`, `run_all_verify.sh` | `node control-studio/scripts/verify_ui_simulation_snapshot_contract.mjs`; `node control-studio/scripts/verify_ui_formula_contract.mjs`; browser DTF effective-loop/formula walkthrough; `npm run verify:all` |
| CS-P0-36 | P0 | Done | UI DTF sample-time codegen/export/project persistence contract | DTF sample time 會由 `DiscreteTransferFunction.sampleTime` 進入 codegen payload、MATLAB/Python preview/export、analysis JSON/Markdown export、autosave/project file 與 local multi-project save/load；project manager 改走 canonical `buildProjectPayload()` / `applyProjectPayload()`，避免 JSON 化 TF class instance 後載入成 plain object | `app.js`, `verify_ui_formula_contract.mjs` | `node control-studio/scripts/verify_ui_formula_contract.mjs`; browser DTF `Ts=0.25` codegen/autosave/project save-load walkthrough; `npm run verify:all` |
| CS-P0-37 | P0 | Done | MATLAB/Python codegen runtime-mode and domain compatibility contract | Generated scripts must match effective runtime mode: open-loop exports plot `G` and never reference undefined `T`; closed-loop exports define/plot `T`; Python exports cannot emit JS booleans; z-domain exports preserve `Ts` and skip continuous PID/L-generation unless a discrete controller is explicitly available | `js/utils/codegen.js`, `app.js`, `verify_codegen_export_contract.mjs`, `run_all_verify.sh` | `node control-studio/scripts/verify_codegen_export_contract.mjs`; browser continuous open-loop + DTF code preview walkthrough; `npm run verify:all` |
| CS-P0-38 | P0 | Done | Discrete analysis export response-type and margin status contract | z-domain analysis export now separates effective `responseType` from `requestedResponseType`; step/impulse route to discrete engines, unsupported requested waveforms normalize to step, discrete impulse exports invalidate step metrics, UI JSON / CLI / FastAPI outputs export non-finite margins as JSON-safe numeric nulls with explicit status fields, and frontend Local/API compare treats status mismatch as a real diff instead of silently ignoring non-finite rows | `app.js`, `control_analysis_cli.mjs`, `verify_discrete_export_response_contract.mjs`, `verify_control_api_contract.mjs`, `verify_api_auto_fallback.mjs`, `run_all_verify.sh` | `node control-studio/scripts/verify_discrete_export_response_contract.mjs`; `node control-studio/scripts/verify_control_api_contract.mjs`; `node control-studio/scripts/verify_api_auto_fallback.mjs`; browser DTF impulse JSON export walkthrough; `npm run verify:all` |
...
| CS-P10-18 | P2 | Done | CI / pre-push verification hook | root `package.json` 加 `verify:math` / `verify:all` / phase-specific scripts；`scripts/git-hooks/pre-push` + `bash scripts/install-hooks.sh` 設 core.hooksPath；失敗 block push（用 `git push --no-verify` 可跳過） | CS-P10-12 / Phase 9-10 runners | `npm run verify:math` 跑核心 math + fixture suite；`npm run verify:all` 跑 131/131 full suite |
...
| CS-P25-03 | P1 | Done | SS minreal / Kalman decomposition | 預設直接 SVD Kalman matrices 移除不可控/不可觀 mode；不形成 Gram products 平方條件數 | state-space core | `verify_p25_model_reduction.mjs` 35/35 |
...
### Phase 76: Deployment Readiness Productization (CS-P76)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-P76-01 | P1 | Done | Deployment readiness gate | 將 codegen / HIL handoff 從「能產生模板」提升為可稽核的工程部署判定；同時檢查 sample time、target artifacts、codegen warnings、traceability、WCET/deadline、jitter、fixed-point headroom、CRC/watchdog/redundancy 與 HIL protocol/channel/sample-time/latency/schema | P33 productization, Tier H/I/J deployment/integration baseline | `node control-studio/scripts/verify_p76_deployment_readiness.mjs`; `npm run verify:all` |

### Phase 77: Deployment Reviewer Skill (CS-P77)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-P77-01 | P1 | Done | `control-studio-deployment-reviewer` skill package | 將 P76 readiness gate 轉成後續 agent 可重用的 codegen / HIL release review workflow，固定輸入清單、必要證據、blocked / conditional / ready 判定、required actions 與 sample input/output | CS-P76 deployment readiness gate, `docs/src/control-studio/skills.md` | `python3 /Users/w.rc/.config/agents/skills/.system/skill-creator/scripts/quick_validate.py skills/control-studio-deployment-reviewer`; `node control-studio/scripts/verify_p77_deployment_skill.mjs`; `npm run verify:all` |

### Zero-Flaw Loop 11: Spectral Subspace Correctness (CS-ZF11)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-ZF11-01 | P0 | Done | General-complex ESPRIT rotation-phase recovery | 舊實作把非對稱 `Phi` 強制 symmetricize，對 multi-tone signal 會丟失複數旋轉相位；50/120 Hz 被錯估為約 47.84/50 Hz。改以 Faddeev-LeVerrier characteristic polynomial + `polyroots()` 取得一般複數 eigenvalues，使用 conjugate pair 的 positive angle 映射 Hz；同時將 MUSIC noise fixture 改為 fixed seed | Loop 9 spectral subspace, polynomial roots | `node control-studio/scripts/verify_loop9_modules.mjs` 13/13；verification Case 12；`npm run verify:all` |

### Zero-Flaw Loop 12: Matrix Diagnostics and Wizard Integration (CS-ZF12)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-ZF12-01 | P0 | Done | True 1-norm condition number and eigenvalue definiteness | 舊 matrix panel 以 max/min row norm 冒充 condition number，且只看正對角線就宣告正定；會把 `[[1,1],[1,1+1e-12]]` 誤判為 κ 約 1，也會把 eigenvalues `3,-1` 的 `[[1,2],[2,1]]` 誤判正定。改用 shared `estimateCondition()` 與 symmetric eigenvalue classification；rectangular/non-symmetric 顯示 N/A | E7 conditioning, matrix eigensolver | `verify_e7_conditioning.mjs`; `verify_p46_b5_b2.mjs` 101/101；verification Case 13 |
| CS-ZF12-02 | P0 | Done | TF / SS / ZPK wizard active-workspace application | 舊 confirm handler 只顯示成功通知，SS/ZPK 未寫回主輸入，且呼叫不存在的 `window.updateSystem`；`_currentSS` 從未建立。新版複製對應輸入、切換 active type、呼叫 module-scoped update，並發布 SS matrix snapshot；matrix parser 也依 UI label 支援 newline / semicolon row separators | P43 wizard, updateSystem, matrix workspace | `verify_p43_syswin_a5.mjs` 88/88；browser walkthrough with zero console errors |

### Zero-Flaw Loop 13: Exact Gramian and HSV Diagnostics (CS-ZF13)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-ZF13-01 | P0 | Done | Shared Lyapunov/Stein Gramian diagnostics | 舊 P39/P55 以截斷 impulse sum 或直接累加 continuous `A^k` 近似 Gramian，TF 路徑還呼叫不存在的 `toStateSpace()`；新版保留 active SS realization，TF 使用 controllable canonical realization，先驗證 Hurwitz/Schur stability，再解 continuous Lyapunov 或 discrete Stein equations，回報 full-matrix eigenvalues、true `kappa_1` 與 relative equation residual | E2 Lyapunov/Stein, E7 conditioning, P39/P55 UI | `verify_p39_uiux_p2_batch2.mjs` 73/73；`verify_p55_b24_b43_b44.mjs` 82/82；verification Case 14 |
| CS-ZF13-02 | P0 | Done | Coupled Hankel singular values via Cholesky-SVD | 舊 UI 用 `sqrt(Wc_ii Wo_ii)`，忽略 off-diagonal controllability/observability coupling。新版由 full Gramians 的 Cholesky factors 計算 singular values；解析反例回傳 `0.7310001561, 0.01899984395`，而不是 diagonal heuristic 的 `0.5, 0.25` | CS-ZF13-01, model reduction core | analytic continuous/discrete fixtures；browser SS wizard + Hankel/Gramian panels；console 0 error |

### Zero-Flaw Loop 14: Balanced Reduction Theory Contract (CS-ZF14)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-ZF14-01 | P0 | Done | Correct square-root balancing SVD orientation | Square-root BT must decompose `Lo^T Lc=U Sigma V^T` before applying `T=Lc V Sigma^-1/2` and `T^-1=Sigma^-1/2 U^T Lo^T`; the former `Lc^T Lo` path used mismatched factors and could violate the BT bound | P25-01, SVD, Lyapunov Gramians | `verify_p25_model_reduction.mjs` 23/23；`verify_p25_hankel.mjs` near-zero-error upper-bound fixture |
| CS-ZF14-02 | P0 | Done | Separate AAK lower bound from BT upper bound | Balanced truncation is not Glover optimal HNA. `balancedTruncationErrorAudit()` reports `sigma_(k+1)` as a lower bound, actual Hankel error, `2 sum sigma_i` as H∞ upper bound, optimality gap, and non-optimal metadata；legacy `hankelNormApprox()` is a compatibility alias | CS-ZF14-01 | counterexample `0.0235744 <= 0.0478577 <= 0.0491548`；`verify_p25_hankel.mjs` 27/27；verification Case 15 |

### Zero-Flaw Loop 15: Hankel Stability and Rank Contract (CS-ZF15)

| ID | Priority | Status | Item | Rationale | Dependencies | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| CS-ZF15-01 | P0 | Done | Hurwitz gate for Hankel metrics and BT | Stable-system Gramians are undefined as energy Gramians for unstable realizations. Former code converted `A=[1]` to fake HSV `1e-10` and mixed `diag(1,-2)` to HSV near `1e10`; all public Hankel/BT paths now reject before solving | CS-ZF13 shared diagnostics, P25 | `verify_p25_hankel.mjs` 29/29；verification Case 16 |
| CS-ZF15-02 | P0 | Done | Preserve exact zero HSVs and numerical rank | Diagonal tolerance regularization created artificial nonzero modes. Cholesky-SVD now preserves exact zeros; diagnostics expose controllability/observability rank + minimality, and BT rejects order above effective Hankel rank while allowing exact active-subspace reduction | CS-ZF15-01, minrealSS | rank-1 fixture HSV `[0.5,0,0]`；`verify_p25_model_reduction.mjs` 31/31 |
| CS-ZF15-03 | P0 | Done | Structural fallback for unstable minreal | `minrealSS()` must use Kalman controllability/observability matrices when A is not Hurwitz instead of interpreting an indefinite Lyapunov solution as an energy Gramian | state-space structural matrices | unstable `diag(1,-2)` fixture preserves active pole `+1` and removes inactive pole |

### Zero-Flaw Loop 16: Structural Rank and Resolution Honesty (CS-ZF16)

| ID | Priority | Status | Deliverable | Blind spot closed | Dependencies | Acceptance evidence |
|---|---:|---|---|---|---|---|
| CS-ZF16-01 | P0 | Done | Direct Kalman-matrix SVD in `minrealSS()` | Forming `Ck*Ck^T` / `Ok^T*Ok` squared condition numbers and deleted weak but valid directions. Default structural path now SVDs `Ck` and `Ok` directly; Gramian rank is explicit opt-in | SVD, state-space Kalman matrices | singular values `1.4142, 7.0711e-6` remain rank 2 at `tol=1e-8`; unstable pole `+2` preserved |
| CS-ZF16-02 | P0 | Done | HSV numerical-resolution floor | Lyapunov error is O(eps), while Gramian factorization introduces an O(sqrt(eps)) HSV floor. Exact cancellation no longer appears as HSV `4.139e-10` | Lyapunov, Cholesky-SVD | exact nonminimal fixture reports zero third HSV and zero BT tail bound |
| CS-ZF16-03 | P0 | Done | Nullable sub-resolution error audit | Near-cancelling block error realizations previously emitted a fake `1.219e-8` actual error and marked it below an `8.279e-10` upper bound via absolute tolerance | CS-ZF16-02, BT audit | `hankelNormError=null`, `hankelNormErrorResolved=false`, nullable comparison flags, explicit resolution `1.142e-8`; verification Case 17 |
