...
- Branch: `main`
- Latest synced checkpoint: `fix(zero-flaw): close discrete export and loop10 verification gaps`
...
| CS-P0-38 | P0 | Done | Discrete analysis export response-type and margin status contract | z-domain analysis export now separates effective `responseType` from `requestedResponseType`; step/impulse route to discrete engines, unsupported requested waveforms normalize to step, discrete impulse exports invalidate step metrics, and non-finite margins export JSON-safe numeric nulls with explicit status fields instead of silently losing `Infinity` / `NaN` semantics | `app.js`, `verify_discrete_export_response_contract.mjs`, `run_all_verify.sh` | `node control-studio/scripts/verify_discrete_export_response_contract.mjs`; browser DTF impulse JSON export walkthrough; `npm run verify:all` |
