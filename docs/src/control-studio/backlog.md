...
| CS-P0-38 | P0 | Done | Discrete analysis export response-type and margin status contract | z-domain analysis export now separates effective `responseType` from `requestedResponseType`; step/impulse route to discrete engines, unsupported requested waveforms normalize to step, discrete impulse exports invalidate step metrics, and UI JSON / CLI / FastAPI outputs export non-finite margins as JSON-safe numeric nulls with explicit status fields instead of silently losing `Infinity` / `NaN` semantics | `app.js`, `control_analysis_cli.mjs`, `verify_discrete_export_response_contract.mjs`, `verify_control_api_contract.mjs`, `run_all_verify.sh` | `node control-studio/scripts/verify_discrete_export_response_contract.mjs`; `node control-studio/scripts/verify_control_api_contract.mjs`; browser DTF impulse JSON export walkthrough; `npm run verify:all` |
