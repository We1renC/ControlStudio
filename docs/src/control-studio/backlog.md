...
- Full suite baseline：`npm run verify:all` / `bash control-studio/scripts/run_all_verify.sh` 目前納入 114 個 deterministic verification scripts，包含 fixture、API contract、runtime UI waveform contract、runtime UI stability snapshot contract、runtime UI simulation snapshot / discrete-domain contract、runtime UI symbol contract 與 n-dimensional equilibrium classification regression；fixture/API contract 目前為 10/10 cases。
...
| CS-P10-18 | P2 | Done | CI / pre-push verification hook | root `package.json` 加 `verify:math` / `verify:all` / phase-specific scripts；`scripts/git-hooks/pre-push` + `bash scripts/install-hooks.sh` 設 core.hooksPath；失敗 block push（用 `git push --no-verify` 可跳過） | CS-P10-12 / Phase 9-10 runners | `npm run verify:math` 跑核心 math + fixture suite；`npm run verify:all` 跑 114/114 full suite |
...
**Validation aggregate status：`control-studio/scripts/run_all_verify.sh` 與 `npm run verify:all` 是目前主驗證入口；full suite 已納入 fixture/API contract、runtime UI waveform contract、runtime UI stability snapshot contract、runtime UI simulation snapshot / freshness / discrete-domain contract、runtime UI symbol contract 與 n-dimensional equilibrium classification regression，最新基線為 114/114；fixture/API contract 為 10/10 cases。P22+ 新增的 P23 / P24 / P25 / P26 / P27 runner 以 roadmap 與 package scripts 為準。**
