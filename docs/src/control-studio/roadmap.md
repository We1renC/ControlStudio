...
**Zero-Flaw Loop 11 closure:** `espritFrequencies()` no longer symmetrizes the non-symmetric LS-ESPRIT rotation matrix `Phi`. It now computes general complex eigenvalues, selects one positive-angle member from each conjugate pair, and maps `arg(lambda_k)` to frequency. This fixes deterministic multi-tone failures where 50/120 Hz was reported as approximately 47.84/50 Hz and 80/92 Hz as approximately 61.93/81.13 Hz. `verify_loop9_modules.mjs` now uses seeded MUSIC noise and checks noiseless, noisy, close-tone, three-tone, and rank-deficient-window ESPRIT cases; verification Case 12 records the derivation and tolerances.
...
**Zero-Flaw Loop 14 closure:** `balancedTruncation()` now applies `T=Lc V Sigma^-1/2` and `T^-1=Sigma^-1/2 U^T Lo^T` to the SVD of `Lo^T Lc`. The former transposed SVD orientation used mismatched `U/V` factors and could violate the balanced-truncation bound. `balancedTruncationErrorAudit()` now labels `sigma_(k+1)` as the AAK lower bound, reports the computed BT Hankel error and `2 sum sigma_i` H-infinity upper bound, and marks the method as non-optimal; `hankelNormApprox()` is retained only as a compatibility alias. A deterministic fixture gives `0.0235744 <= 0.0478577 <= 0.0491548`.

**Zero-Flaw Loop 15 closure:** public Hankel metrics and balanced truncation now require a Hurwitz realization. The former code could turn a negative unstable Gramian into a small positive HSV through diagonal regularization, or return an approximately `1e10` HSV for a mixed stable/unstable realization. Exact zero-energy modes now remain zero; diagnostics expose controllability/observability rank and minimality, balanced truncation rejects orders above Hankel numerical rank, and unstable `minrealSS()` uses structural Kalman matrices.

**Zero-Flaw Loop 16 closure:** `minrealSS()` now defaults to direct SVD of Kalman controllability/observability matrices instead of Gram products that square the condition number. Weak rank-2 fixtures retain unstable pole `+2` at `tol=1e-8`; stable Gramian rank is explicit opt-in. HSV rank uses a `sqrt(machine epsilon)` floor, and balanced-truncation error audit returns nullable unresolved actual-error fields below that resolution while preserving theorem bounds.

**Zero-Flaw Loop 17 closure:** `minrealSS()` validates matrix shapes/options, supports explicit zero-state systems, and returns canonical `A=[]`, `B=[]`, `C=ny x 0`, `D=ny x nu` for pure feedthrough MIMO models. Dense similarity-transform regression preserves the order-2 transfer map from a nonminimal order-3 realization with `max|G-G_r|=5.62e-15`.

**Zero-Flaw Loop 18 closure:** balanced truncation/error audit reject fractional order, malformed or non-finite matrices, and invalid tolerance before numerical work. Reduced and minreal feedthrough matrices are deep copies, preventing result mutation from changing the source plant.
...
| P25-02 Hankel metrics + balanced-truncation error audit | Done | Hurwitz/rank guards, exact zero-HSV, nullable unresolved-error, and reduction input contracts; P25 runners 49/49 and 29/29 |
| P25-03 SS minreal / Kalman decomposition | Done | direct `Ck`/`Ok` SVD, zero-state MIMO shape, similarity transfer equivalence, non-aliasing D; `verify_p25_model_reduction.mjs` 49/49 |
