...
| --- | --- | --- | --- | --- | --- |
| Phase 18 | P1 | Next | Uncertainty + Monte Carlo robust validation | 將 robust analysis 從 nominal 指標推進到不確定性族群驗證 | deterministic uncertainty fixtures、worst-case replay、`verify:p18` |
...
1. 先開 Phase 18：uncertainty + Monte Carlo robust validation。
2. 同步建立第一個 skill：`control-studio-robust-validator`。
3. Phase 18 完成後，再建立 `control-studio-system-auditor` 與 `control-studio-benchmark-author`，把驗證流程標準化。
