...
test -f "$ROOT_DIR/docs/src/agents/token-efficiency.md"
test -f "$ROOT_DIR/docs/agents/token-efficiency.html"
test -f "$ROOT_DIR/requirements-agent-tools.txt"
...
grep -q "Verification fixtures passed: 10/10" /tmp/nvidia-control-verification-cases.txt
grep -q "API contract fixtures passed: 10/10" /tmp/nvidia-control-api-contract.txt
grep -q "Verification fixtures passed: 10/10" /tmp/nvidia-control-npm-verify-math.txt
