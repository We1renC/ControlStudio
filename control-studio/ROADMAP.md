...
> Current committed baseline: `fix(control): gate non-step response metrics`
...
**111/111 scripts pass** — run via `bash scripts/run_all_verify.sh` or `npm run verify:all` (was 82/82 before Functional Roadmap additions). Fixture/API contract coverage is now **7/7 cases**, including open-loop controller cascade response and non-step waveform metrics gating.
