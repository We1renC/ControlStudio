...
assert(appJs.includes('function computeCurrentGramianDiagnostics()'), 'shared Gramian diagnostic route defined');
assert(appJs.includes('gramianDiagnostics(ss.A, ss.B, ss.C, ss.D'), 'Hankel panel uses exact Gramian core');
assert(appJs.includes('Lyapunov') && appJs.includes('Stein'), 'continuous/discrete equation type displayed');
assert(appJs.includes('controllabilityResidual'), 'equation residual displayed');
assert(!appJs.includes('Simple power-method Gramian approximation'), 'power-method HSV approximation removed');
assert(!appJs.includes('Extract diagonal as approximation of singular values'), 'diagonal HSV heuristic removed');
